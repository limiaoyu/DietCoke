...
def get_logger(root_dir):
    ...
    handler = logging.FileHandler(root_dir + "log_knowledge.txt")
...
def create_messages(Context_Prompt, Task_Prompt, question):
    ...
    messages.append({"role": "assistant", "content": "I don't have enough knowledge to answer this question."})
    messages.append({"role": "user", "content": "Please provide background knowledge related to this question in a single sentence."})
    return messages
...
def evaluation(model, test_data,caption_dict,syn_question_dict,syn_answer_dict,ans_to_cap_dicts,tokenizer, device, config, logger, writer, split='test',resume=False,result_dir = 'output/VQA_caption/result', result_filename = 'result'):
    ...
    for n, per_test_data in enumerate(metric_logger.log_every(test_data, print_freq, header)):
        ...
        messages = create_messages(Context_Prompt, Task_Prompt, question)
...
def main(args, config, logger, writer):
    ...
    if not args.evaluate_direct:
        ...
        print('save results')
        with open(config['output_dir'] + "knowledge.json", 'w') as json_file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default='./output_knowledge_aok/mistral_inst/short/')
