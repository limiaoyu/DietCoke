...
def load_model(model_selection,dist_selection):
    ...
    if model_selection == 'mistral-7b-inst':
...
def evaluation(model, test_data,caption_dict,syn_question_dict,syn_answer_dict,ans_to_cap_dicts,tokenizer, device, config, logger, writer, split='test',resume=False,result_dir = 'output/VQA_caption/result', result_filename = 'result'):
    ...
    # print(test_data)
    answer_data = json.load(open(config['answer_file'],'r'))
    index = 0
    ...
    for n, per_test_data in enumerate(metric_logger.log_every(test_data, print_freq, header)):
        ...
        cur_answer = answer_data[index]['answer']
        index += 1
        ...
        messages = create_messages(Prompt, Context_Prompt, Task_Prompt, question, cur_answer)
        ...
        pred_answer = decoded[0].split("[/INST]")[-1]
        pred_answer = pred_answer.split("</s>")[0]
        pred_answer = pred_answer.replace('\n', ' ')

        process_logger.info(f"ques_id: {question_id}  question: {question}  pred_answer: {cur_answer}  context: {Context_Prompt}  rationale: {pred_answer}\n")
        result[str(question_id)] = pred_answer
    return result
...
def update(params, args):
    ...
    params['answer_file'] = args.answer_file
...
if __name__ == '__main__':
    ...
    parser.add_argument('--answer_file', default='./output_candidate_aok/short/result/_aokvqa_mistral-7b_huggingcaps30_question30_questiontype_g_q.json')
    ...
    parser.add_argument('--output_dir', default='./output_rationale_aok/automatic/long/')
    ...
    parser.add_argument('--model_selection', default='mistral-7b-inst', type=str)
