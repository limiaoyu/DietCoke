...
class GenerationMixin:
    ...
    def greedy_search(self, *args, **kwargs):
        ...
        return self._greedy_search_new(*args, **kwargs)
    ...
    def _greedy_search(
        ...
        **model_kwargs,
        ...
        >>> outputs = model._greedy_search_new(
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=True,  # grad_cam
            ...
            with torch.no_grad():
                mask = model_inputs['attention_mask'].view(1, 1, 1, -1).repeat(1, 32, 1, 1)
                grads = self.base_model.layers[-1].get_attn_gradients()
                grads = grads.clamp(min=0).cuda(0)
                cams = outputs['attentions'][-1].cuda(0)
                grads = grads * mask
                cams = cams * mask
                gradcams = cams * grads
                gradcam_list.append(gradcams)
        ...
        gradcam_list_new = []
        for gradcams in gradcam_list:
            gradcam_list_new.append(gradcams.mean(1).view(-1)[:prompt_length])
        gradcam_list = torch.sum(torch.stack(gradcam_list_new), dim=0)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH:
            ...
            result, gradcam_list = self._greedy_search(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
